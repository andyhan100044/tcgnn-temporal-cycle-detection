...
表~\ref{tab:main} 报告 Elliptic1 上 1{,}000 候选环（500 注入正 + 500 约束违反负）下的环级检测（扩展曲线见表~\ref{tab:scaling}）。

\begin{table}[t]
\centering
\caption{真实 Elliptic1 + 注入环的环级检测（SQLite 后端，1{,}000 候选，10 epoch）。粗体：约束驱动 GNN 系列中最佳。$\dagger$：仅用节点图特征。}
\label{tab:main}
\small
\begin{tabular}{lcccccc}
\toprule
...
\textbf{讨论。} TC-GNN 在真实 Elliptic1 上达到 AUC-ROC $0.776$——相比纯合成实验（表~?）提升 $44\%$，证实约束驱动架构可泛化至合法交易拓扑。GAT 以 $0.878$ 的 AUC-ROC 在静态图基线中表现最佳，而 DCRNN 和 GLASS 特别善于利用环的时间有序边序列。XGBoost 的完美分数反映了我们的约束违反负样本具有手工特征（长度、金额、时间跨度），与价值守恒的正样本相比平凡可分——这是实验设置的特点而非模型缺陷。我们在第~\ref{sec:conclusion}节讨论这种权衡的局限。

\subsection{可扩展性}
\label{sec:exp-scaling}

我们将候选集从 1{,}000 扩展到 5{,}000（表~\ref{tab:scaling}）。TC-GNN 在 AUC-ROC $\sim 0.7$ 处趋于平稳，因为约束违反负分布与约束满足正分布变得平凡可分；额外负样本不增加新学习信号。DCRNN 和 XGBoost 保持优势。我们将此解释为瓶颈现在是\emph{负样本质量}（我们使用违反约束的随机游走），而非数量。未来工作将从真实近失配模式（例如看起来几乎环形的合法供应商付款）中抽取负样本。

\begin{table}[t]
\centering
\caption{扩展性：候选数增长时 TC-GNN 的 AUC-ROC。}
\label{tab:scaling}
\small
\begin{tabular}{lccccc}
\toprule
\textbf{候选数} & \textbf{正} & \textbf{负} & \textbf{TC-GNN} & \textbf{DCRNN} & \textbf{XGBoost} \\
\midrule
