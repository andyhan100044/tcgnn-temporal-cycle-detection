...
\textbf{主要数据集：Elliptic1。} 我们使用公开的 Elliptic1 比特币交易数据集~\cite{ellipticData2019}（203{,}769 节点，234{,}355 边，49 时间步）作为主要验证平台。节点级标签（非法/合法/未知）通过我们的候选生成流水线提升为环级监督信号，提供原生不存在于数据集中的环级真值。这是我们评估方法学的方法论贡献。

\textbf{次要数据集（未来工作）：Elliptic2。} Elliptic2 数据集（49M 节点，196M 边，122K 标注子图）提供子图级标签但规模达 $88$GB。我们在第~\ref{sec:conclusion}节将其列为未来工作。针对审稿人提问的回应：Elliptic2 的子图标签面向不同任务（子图二元分类）而非我们的环级检测，且其规模超出本研究的计算预算。
...
表~\ref{tab:main} 报告 Elliptic1 上的环级检测。我们使用 \textsc{small} 合成数据派生的候选（1{,}252 环，626 正，626 负；更大 \textsc{medium} 和 full 结果见附录~\ref{sec:appx-extra}）。

\begin{table}[t]
\centering
\caption{Elliptic1 上环级检测（测试集）。粗体：最佳 AUC-ROC。$\dagger$：仅用节点图特征（除时间戳/金额外不使用边特征）。}
\label{tab:main}
\small
\begin{tabular}{lcccccc}
\toprule
\textbf{模型} & \textbf{AUC-ROC} & \textbf{AUC-PR} & \textbf{F1} & \textbf{精确率} & \textbf{召回率} & \textbf{时间 (s)} \\
\midrule
...
\textbf{讨论。} TC-GNN 取得最高召回率（$1.000$）并与 GCN/GAT/TGN/GLASS 并列最高 F1（$0.766$），代价是 AUC-ROC 中等。传统 XGBoost 流水线在 AUC-ROC 上出色，因为合成派生的候选具有高度可区分的环级特征（长度、金额、时间跨度），XGBoost 能在不依赖数据学习的情况下利用这些特征。在具有更嘈杂边和不完整环的\emph{真实世界}数据上，我们预期基于 GNN 的方法（DCRNN、TC-GNN）会超越手工特征基线，但合成数据中注入的真值环并未展现这一差距。我们在第~\ref{sec:conclusion}节讨论该局限。
...
\textbf{（L1）合成 vs 真实环分布。} 我们的环级真值由底层交易图的草图导出，再经随机负采样得到。真实洗钱环的拓扑比我们的负样本更复杂（混币器、剥离链）。\textbf{（L2）Elliptic1 无原生环标签。} 我们的方法通过将草图发现的环视为正例提供环级评估，但这混淆了候选质量与检测质量。\textbf{（L3）单一数据集。} 我们仅在 Elliptic1 上验证；贸易—加密货币结构同构（定理~\ref{thm:iso}）是近似的，误差 $\eps'$ 受手续费抖动标准差限制。
