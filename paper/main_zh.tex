...
\paragraph{标注构建的局限性与弱监督框架声明。}

我们坦诚承认评估方法学中三类标注构建风险。\textbf{（L'）定义偏差}：我们用以构造环的规则（非法节点邻域 + 时间递增 + 价值守恒）是我们\emph{假设的}洗钱模式，不是 ground truth。\textbf{（L''）样本偏差}：正样本仅从 4{,}545 个非法节点（占图 2.2\%）构建，造成类别不平衡。\textbf{（L'''）验证缺失}：Elliptic1 不存在独立的环级金标准。

为缓解这些风险，我们将环级标注\emph{框定为弱监督信号}而非 ground truth。环级伪标签由透明的业务规则启发式（节点来源 + 时间 + 价值约束）导出，\emph{与强监督的节点级标签联合使用}于模型训练与评估。这与图上弱监督学习的标准实践一致~\cite{inspectionL2024,lawal2025}。未来工作（第~\ref{sec:conclusion-future}节 F6）将以\emph{人工验证的环级金标准}（如来自执法案件档案）或\emph{更大规模的子图级标注数据集}（如 Elliptic2）替换启发式。

\textbf{负类（约束违反的游走）。} 真实 Elliptic1 几乎不含自然的时间递增环，因此我们将负类生成为违反 C1--C5 中一条或多条的随机游走。分类器必须区分约束满足的环与约束违反的随机游走——一种``红队''压力测试。

\textbf{次要数据集（未来工作）：Elliptic2。} Elliptic2 数据集（49M 节点，196M 边，122K 标注子图）提供子图级标签但规模达 $88$GB。我们在此开发的 SQLite 后端可通过相同的索引查询流式处理 Elliptic2；完整的 Elliptic2 评估作为第~\ref{sec:conclusion}节的未来工作。我们将范围限定于此，因为 Elliptic2 的子图标签面向不同任务（子图二元分类）而非我们的环级检测，且其 $88$GB 规模超出本研究的计算预算。
...
我们在测试集上报告 AUC-ROC、AUC-PR 和 F1。AUC-PR 是类别不平衡（我们的设定下 $1.6{:}1$）下的头条指标。我们不将``约束满足率''作为头条对比指标，因为它对 TC-GNN 是自证性的；我们仅在消融诊断中保留。
...
\textbf{（L1）合成环注入。} 正类在比特币交易中并非自然存在（其构成 DAG）。我们注入合成洗钱环以评估检测。虽然这能将模型从自然背景效应中隔离出来，但可能低估真实洗钱环的复杂性（混币器、剥离链、Gas 费混淆）。

\textbf{（L2）约束满足的负样本\emph{看起来像正样本}。} 我们的二级负类使用现实 AML 假阳性模式：退款链、交易所路由、时间扰动的合法流、长环近失配（表~\ref{tab:realistic}）。在更具挑战性的评估下，TC-GNN 的 AUC-ROC 从 $0.962$ 跌至 $0.530$——统计上与随机无显著差异。原因是结构性的：TC-GNN 的硬约束嵌入无法区分在非法节点上的约束满足环和合法节点上的约束满足环。结合约束感知与丰富节点特征的模型（GAT、GCN、GLASS）保持其领先优势（$0.87$--$0.99$），因为它们利用 165 维 Elliptic1 特征。我们在第~\ref{sec:conclusion-future}节中将其作为 \textsc{TC-GNN}2 的未来工作。\textbf{（L5）环级标签是弱监督而非 ground truth}。我们通过透明的业务规则启发式构造环正例（非法节点邻域 + 时间递增 + 价值守恒）。此启发式可能\emph{遗漏}违反 C2--C4 的非显然真实洗钱模式，或\emph{虚构}形似洗钱但非洗钱的模式。我们将评估框定为弱监督研究（第~\ref{sec:exp-data}节），将人工验证的金标准列为未来工作（第~\ref{sec:conclusion-future}节 F6）。

\subsection{跨域移植：从加密货币到国际贸易}
\label{sec:transfer}

在加密货币（Elliptic1）上的验证是\emph{沙盒}验证；实际部署目标是国际贸易欺诈检测。直接移植存在困难，因为两个域在多个维度上存在根本差异，破坏了我们的硬约束嵌入。我们识别三个具体缺陷，必须在运营部署前解决。

\textbf{缺陷 T1：价值守恒 $\neq$ 价值偏差检测。} 在加密货币洗钱中，行为人追求\emph{价值守恒}（存入 1 BTC，取出 1 BTC 减混币费），所以我们的约束 C4 匹配真实行为模式。在贸易洗钱（TBML）中，行为人\emph{希望}申报价值与成本偏离：例如 \$1M 出口 $\to$ \$1.2M 再进口 $\to$ \$1.5M 再出口，每一跳都\emph{虚增}价值以套取更大退税差额。我们设计 TC-GNN 来检测的特征（价值稳定）正是 TBML 的错误信号。直接移植会系统性漏报。\textbf{缓解措施：} 将\emph{价值守恒}替换为\emph{价值偏差异常度}（例如标记偏离该商品行业平均加价 $> 2\sigma$ 的边）。

\textbf{缺陷 T2：时间窗口的语义跨数量级不同。} 加密货币闭环在分钟到小时内完成（$\Delta t \sim 24$ 小时由混币器批次窗口合理化）。贸易型循环必须对齐\emph{申报周期}（月度 VAT 申报、季度出口退税申请）和\emph{物流运输时间}（航运数周、库存数月）。为加密货币设置的 $\Delta t$ 错过所有跨月贸易环；为贸易设置的 $\Delta t$ 计算上不可行（$\Delta^k$ 爆炸）。\textbf{缓解措施：} 将 $\Delta t$ 参数化为\emph{申报周期单位}而非绝对时间，并利用问题结构（如季末的公司间交易聚集）剪枝。

\textbf{缺陷 T3：缺少物理世界交叉验证。} 加密货币交易是\emph{纯资金流}；链上闭环即完整行为。贸易交易有\emph{并行的物理流}（物流、报关单、库存记录、发票），可外部审计。真正空的贸易型循环（无实际货物运输）在金融图（构成循环）与物理图（不构成循环）之间产生\emph{流错配}。TC-GNN 仅建模金融图，无法检测。\textbf{缓解措施：} 未来工作（第~\ref{sec:conclusion-future}节 F7）应构建\emph{异构图}，同时包含\emph{资金流边}与\emph{货物流边}，在双部图投影中检测两个流类型本应一致的循环。

\textbf{移植可行但非平凡。} 上述缺陷\emph{不是} TC-GNN 框架的固有局限——它们是将加密货币验证的框架移植到结构不同域时的\emph{适配挑战}。每个缺陷都有已知的数学补救（用偏差异常度替换 C4；重参数化 $\Delta t$；用货物流边增广图）。因此我们将 TC-GNN 定位为\emph{环检测框架}，其\emph{约束层}是可复用的智力贡献，其\emph{约束值}需按域重新设置。

\textbf{（L3）SQLite 内存约束，非理论限制。} 我们的 SQLite 后端可在普通硬件上扩展到 5{,}000 候选；Elliptic2（49M 节点）将需要相同架构配以更大磁盘预算和流式训练。\textbf{（L4）单一真实数据集。} 我们仅在 Elliptic1 上验证；贸易—加密货币结构同构（定理~\ref{thm:iso}）是近似的，误差 $\eps'$ 受手续费抖动标准差限制。

\subsection{压力测试：现实 AML 假阳性模式}
\label{sec:exp-stress}

为应对"近失配负样本仍可与正样本分离"的担忧，我们使用源自 FATF 类型学~\cite{fatfTypologies2022}的四种模式生成\emph{现实 AML}负类：

\begin{itemize}
\item \textbf{退款链}（$k{=}2$）：客户向商户付款，商户退款——交易所数据中最常见的 AML 假阳性。
\item \textbf{交易所路由}（$k{=}4$）：$A \to \text{交易所} \to B \to C \to A$ 通过中心枢纽。
\item \textbf{时间扰动的合法环}：与正样本相同结构但具有现实业务时间尺度（数周到数月）。
\item \textbf{长环近失配}（$k{=}5$--$7$）：具有多个中介的复杂合法业务流。
\end{itemize}

\begin{table}[t]
\centering
\caption{现实 AML 负样本上的压力测试。TC-GNN 的硬约束嵌入崩溃，因为许多约束满足的环是合法的。GAT/GCN/GLASS 通过 165 维节点特征保持其优势。}
\label{tab:realistic}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{模型} & \textbf{近失配 AUC-ROC} & \textbf{现实 AUC-ROC} & $\Delta$ \\
\midrule
...
TC-GNN 在现实负样本下的崩溃是结构性限制，而非超参数问题。TC-GNN 验证\emph{形状约束}（C1--C5）但不推理\emph{来源}（哪些节点携带非法标签）。这激励我们的未来工作 \textsc{TC-GNN}2（第~\ref{sec:conclusion-future}节），它将结合硬约束嵌入与来源感知的节点特征。

\subsection{未来工作}
\label{sec:conclusion-future}

\textbf{（F1）Elliptic2 验证。} 对 49M 节点的 Elliptic2 图采样子图，测试 TC-GNN 在子图级标签上的可扩展性。\textbf{（F2）多模态贸易数据。} 与海关总署合作（脱敏报关数据），将 TCD 应用于真实贸易流数据。\textbf{（F3）联邦学习。} 通过联邦时序图学习实现隐私约束下的多银行反洗钱检测。\textbf{（F4）自适应约束阈值。} 从数据中学习 $\Delta t$ 和 $\eps$，而非固定。\textbf{（F5）\textsc{TC-GNN}2 与来源感知。} 将硬约束嵌入（C1--C5）与来源感知的节点特征结合，区分基于非法锚定的循环与基于合法锚定的循环——应对我们压力测试（第~\ref{sec:exp-stress}节）暴露的结构性限制。\textbf{（F6）弱监督 TC-GNN 与环级金标准。} 用人工验证的环级标注（如来自执法案件档案）或 Elliptic2 的子图级标签替换业务规则环标签。将训练框定为多任务：$\L_{\text{node}}$（4{,}545 非法节点的强监督） + $\L_{\text{cycle}}$（候选生成的弱监督） + $\L_{\text{consistency}}$（跨粒度一致性正则化）。\textbf{（F7）多模态异构图用于贸易数据。} 用\emph{货物流}与\emph{票据流}边增广金融图。在\emph{双部图投影}中检测循环——金融循环 $\Leftrightarrow$ 物理非循环（即钱循环而货不循环）。这直接应对\textbf{缺陷 T3}（第~\ref{sec:transfer}节）。\textbf{（F8）分层 TC-GNN 用于超长循环。} 将 $k > 12$ 的循环分解为重叠的 3--5 循环子循环，跨层级聚合预测。\textbf{（F9）域自适应约束松弛。} 从小规模标注集自动学习每域约束阈值（$\Delta t$, $\eps$）甚至约束\emph{类型}（守恒 vs.\ 偏差），以最小化手动重工程实现跨域迁移。
